...
public class WeaponScript : MonoBehaviour {
	...
	void Shoot () {
		...
		{
			RaycastHit hit;
			if(Physics.Raycast(camera.position, transform.forward, out hit, Mathf.Infinity))
			{
				...
				if(hit.transform.tag == "Body")
				{
					if(damagable != null)
				{
					float normaldistance = hit.distance / maxRange;
					if (normaldistance <= 1)
					{damagable.DealDamage(Mathf.RoundToInt(Mathf.Lerp(maxDamage, minDamage, normaldistance)));}
				}
				}
					...
					if(damagable != null)
				{
					float normaldistance = hit.distance / maxRange;
					if (normaldistance <= 1)
					{	int damage = Mathf.RoundToInt(Mathf.Lerp(maxDamage, minDamage, normaldistance));
